...
func TestPlot(t *testing.T) {
	//println(os.Getenv("MASS_CI"))
	//testutil.SkipCI(t)

	var bl = 24
	var space_dir = "testdata"
	...
	fileInfo, err := os.Stat(space_dir)
	if nil == fileInfo && err != nil && !os.IsExist(err) {
		os.Mkdir(space_dir, os.ModePerm)
	}

	mdb, err := massdb_v1.NewMassDBV1(space_dir, engine.UnknownOrdinal, pk, bl)
	if err != nil {
		t.Error(err)
		t.FailNow()
	}
